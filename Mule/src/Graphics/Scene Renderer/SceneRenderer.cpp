...
namespace Mule
{
	...
	void SceneRenderer::RefreshEngineObjects()
	{
		...
		auto normalTexture = mAssetManager->GetAsset<Texture2D>(MULE_NORMAL_TEXTURE_HANDLE);
		mNormalImageIndex = mTextureArray.Insert(whiteTexture->Handle(), normalTexture);
	}
