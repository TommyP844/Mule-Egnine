...
namespace Mule
{
	...
	void VulkanCommandBuffer::TransitionSwapchainFrameBufferForRendering(Ref<SwapchainFrameBuffer> fb)
	{
		// Color Attachment
		{
	...
	void VulkanCommandBuffer::TransitionSwapchainFrameBufferForPresent(Ref<SwapchainFrameBuffer> fb)
	{
		// Color Attachment
		{
			VulkanImage& imageData = fb->GetColorImage();
	...
	void VulkanCommandBuffer::BeginRenderPass(Ref<SwapchainFrameBuffer> framebuffer)
	{
		VkRect2D rect{};
		rect.offset.x = 0;
		rect.offset.y = 0;
	...
	void VulkanCommandBuffer::BeginRenderPass(WeakRef<VulkanFrameBuffer> framebuffer, WeakRef<VulkanGraphicsShader> shader)
	{
		VkRect2D rect{};
		rect.offset.x = 0;
		rect.offset.y = 0;
	...
	void VulkanCommandBuffer::ClearFrameBuffer(WeakRef<VulkanFrameBuffer> framebuffer)
	{
		for (uint32_t i = 0; i < framebuffer->GetColorAttachmentCount(); i++)
		{
			auto attachment = framebuffer->GetColorAttachment(i);
	...
	void VulkanCommandBuffer::TranistionImageLayout(WeakRef<IVulkanTexture> texture, ImageLayout newLayout)
	{
	...
	void VulkanCommandBuffer::CopyTexture(WeakRef<IVulkanTexture> src, WeakRef<IVulkanTexture> dst, const TextureCopyInfo& copyInfo) const
	{
		VkImageCopy region{};
	...
	void VulkanCommandBuffer::ReadTexture(WeakRef<IVulkanTexture> texture, uint32_t x, uint32_t y, uint32_t width, uint32_t height, WeakRef<VulkanStagingBuffer> buffer) const
	{
	...
	void VulkanCommandBuffer::BindComputePipeline(WeakRef<VulkanComputeShader> shader)
	{
		vkCmdBindPipeline(mCommandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, shader->GetPipeline());
	}

	void VulkanCommandBuffer::SetPushConstants(WeakRef<VulkanGraphicsShader> shader, ShaderStage stage, const void* data, uint32_t size)
	{
	...
	void VulkanCommandBuffer::SetPushConstants(WeakRef<VulkanComputeShader> shader, void* data, uint32_t size)
	{
		vkCmdPushConstants(mCommandBuffer, shader->GetPipelineLayout(), VK_SHADER_STAGE_COMPUTE_BIT, 0, size, data);
	}

	void VulkanCommandBuffer::BindGraphicsDescriptorSet(WeakRef<VulkanGraphicsShader> shader, const std::vector<WeakRef<VulkanDescriptorSet>>& descriptorSets)
	{
	...
	void VulkanCommandBuffer::BindComputeDescriptorSet(WeakRef<VulkanComputeShader> shader, WeakRef<VulkanDescriptorSet> descriptorSet)
	{
