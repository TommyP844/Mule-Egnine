#include "Graphics/API/Vulkan/VulkanStagingBuffer.h"
#include "Graphics/Context/GraphicsContext.h"
#include <spdlog/spdlog.h>

namespace Mule
{
	VulkanStagingBuffer::VulkanStagingBuffer(WeakRef<GraphicsContext> context, uint32_t size)
		:
		IVulkanShaderBuffer(context->GetDevice()),
		mSize(size)
	{
	...
	void VulkanStagingBuffer::SetData(const void* data, VkDeviceSize size, VkDeviceSize offset)
	{
		void* mappedData;
	...
	Buffer VulkanStagingBuffer::ReadData()
	{
		Buffer buffer(mSize);
